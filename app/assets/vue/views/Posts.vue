<template>
    <div>
        <div class="row col">
            <h1 v-text="$ml.with('VueJS').get('titlePosts')"></h1>
        </div>
        ...
        <div class="row col" v-if="canCreatePost">
            <form>
                <div class="form-row">
                    ...
                    <div class="col-4">
                        <button @click="createPost()" :disabled="message.length === 0 || isLoading" type="button" class="btn btn-primary" v-text="$ml.with('VueJS').get('createBtn')"></button>
                    </div>
                </div>
            </form>
        </div>
